...
function parseUA(ua: string): UAInfo {
  ...
  let browser = "Unknown"; let browserVersion = "";
  let os = "Unknown"; let osVersion = "";
  let engine = "Unknown";
  ...
  else if (/MSIE ([0-9.]+)/i.test(ua) || /Trident\//i.test(ua)) { browser = "Internet Explorer"; browserVersion = ua.match(/(?:MSIE |rv:)([0-9.]+)/i)?.[1] ?? ""; }
  ...
  } else if (/iPhone OS ([0-9_]+)/i.test(ua) || /iPad.*OS ([0-9_]+)/i.test(ua)) {
  ...
  return {
    ...
    os, osVersion,
    device: tablet ? "Tablet" : mobile ? "Smartphone" : "Desktop",
    type: mobile || tablet ? "Mobile" : "Desktop",
    engine,
    raw: ua,
  };
}
...
export function UserAgentParserTool() {
  ...
  const [info, setInfo] = useState<UAInfo | null>(MY_UA ? parseUA(MY_UA) : null);

  const parse = () => { setInfo(parseUA(ua)); };
  const useMyUA = () => { setUa(MY_UA); setInfo(parseUA(MY_UA)); };
    ...
    <div className="rounded-2xl border border-border bg-card overflow-hidden">
      ...
      <div className="p-5 space-y-4">
        <div className="flex gap-2">
          <Input value={ua} onChange={e => setUa(e.target.value)} className="text-xs font-mono flex-1" placeholder="Paste a user agent string…" />
          <Button onClick={parse} className="shrink-0">Parse</Button>
          <Button onClick={useMyUA} variant="outline" className="shrink-0 text-xs">My UA</Button>
        </div>

        {info && (
          <div className="space-y-2">
            {rows.map(([label, value]) => (
              <div key={label} className="flex items-start gap-3 p-3 rounded-xl border border-border bg-muted/10">
                <span className="text-xs text-muted-foreground w-36 shrink-0">{label}</span>
                <span className="text-sm font-medium">{value}</span>
              </div>
            ))}
            <div className="p-3 rounded-xl border border-border bg-muted/10">
              <p className="text-xs text-muted-foreground mb-1">Raw UA String</p>
